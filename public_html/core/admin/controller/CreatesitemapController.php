...
class CreatesitemapController extends BaseAdmin
{
	...
	protected $fileArr = ['jpg', 'png', 'jpeg', 'gif', 'xls', 'xlsx', 'pdf', 'mp4', 'mpeg', 'mp3', 'ogg', 'ogv'];
	protected $filterArr = [
		'url' => ['order', 'page'],
		'get' => []
	];
	...
	public function inputData($links_counter = 1, $redirect = true){
		...
		if(!function_exists('curl_init')){
            ...
            $this->cancel(0, 'CURL library is missing. Creating a sitemap is not possible', '', true);
		...
		if ($redirect){
            ...
            if(!$_SESSION['res']['answer']){
                $_SESSION['res']['answer'] = '<div class="success">Sitemap is created</div>';
            }
        ...
        do{
            ...
            if (false !== $info){
                ...
                if ($info['result'] !== 0){
                    ...
                    $headers = curl_getinfo($curl[$i]);

                    if ($error !== 0){

                        $this->cancel(0, 'Error loading ' . $headers['url'] .
            ...
            if ($links[2]){
                ...
                foreach ($links[2] as $link){
                    ...
                    if (!in_array($link, $this->bad_links) && !preg_match('/^(' . $site_url . ')?\/?#[^\/]*?$/ui', $link)
                        ...
                        $this->temp_links[] = $this->all_links[] = $link;
	...
	protected function createSitemap(){
        ...
        if ($this->all_links){
            ...
            foreach ($this->all_links as $item){
                ...
                $date = new \DateTime();
                $urlMain->addChild("lastmod",$date->format("Y-m-d").'T'.$date->format("H:i:s+03:00"));
                $urlMain->addChild("changefreq","daily");
                $urlMain->addChild("priority", $priority);

            }
//            $urlMain = $sxe->addChild("url");
//            $urlMain->addChild("count", count($this->links_arr));
        }
