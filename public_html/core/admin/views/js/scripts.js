                ...
                for (let i in this.files) {
                    ...
                    if (this.files.hasOwnProperty(i)){
                        ...
                        if (multiple){
                            ...
                            container[i].setAttribute(`dataDeleteFileId-${attributeName}`, elId);
...
function changeMenuPosition() {
    ...
    if (form){
        ...
        let selectMenuPosition = form.querySelector('select[name=menu_position]');

        if (selectParent && selectMenuPosition){

            let defaultParent = selectParent.value;
            let defaultMenuPosition = +selectMenuPosition.value;

            selectParent.addEventListener('change', function () {

                let defaultChose = false;

                if (this.value === defaultParent) defaultChose = true;
                ...
                Ajax({
                    data:{
                        ...
                        ajax: 'change_parent',
                        iteration: !form.querySelector('#tableId') ? 1 : +!defaultChose
                    }
                }).then(res =>{
                    ...
                    for (let i = 1; i <= res; i++){
                        ...
                        let selected = defaultChose && i ===defaultMenuPosition ? 'selected' : '';

                        newSelect.insertAdjacentHTML('beforeend', `<option ${selected} value="${i}">${i}</option>`)

                    }

                    selectMenuPosition.before(newSelect);

                    selectMenuPosition.remove();

                    selectMenuPosition = newSelect;
                        ...
                        if (container.children.hasOwnProperty(i)){
                            ...
                            if (!container.children[i].matches('label') && !container.children[i].matches('.empty_container')){
                                ...
                                if (container.children[i].tagName === 'A'){
                                    ...
                                    res.push(container.children[i].getAttribute(`datadeletefileid-${name}`))
