...
class Crypt
{
    ...
    protected function cryptCombine($str, $iv, $hmac){
        ...
        $counter = (int)ceil(strlen(CRYPT_KEY) / ($str_len + $this->hashLenght));
	...
	protected function cryptUnCombine($str, $ivlen){
		...
		$hash_position = (int)ceil(strlen($str) / 2 - $this->hashLenght / 2);
	
		$crypt_data['hmac'] = substr($str, $hash_position, $this->hashLenght);
		
		$str = str_replace($crypt_data['hmac'], '', $str);
		
		$counter = (int)ceil(strlen(CRYPT_KEY) / (strlen($str) - $ivlen + $this->hashLenght));
